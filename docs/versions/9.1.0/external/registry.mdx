...
*   `/modules/$MODULE/$VERSION`: A directory containing the following files:
    *   `MODULE.bazel`: The `MODULE.bazel` file of this module version. Note
        ...
        override](/versions/9.1.0/external/module#non-registry_overrides)). Also note that it's
        ...
        versioning, [see the FAQ](/versions/9.1.0/external/faq#module-versioning-best-practices).
...
*   `yanked_versions`: A JSON object specifying the [*yanked*
    versions](/versions/9.1.0/external/module#yanked_versions) of this module. The keys
...
*   If `type` is `archive` (the default), this module version is backed by an
    [`http_archive`](/versions/9.1.0/rules/lib/repo/http#http_archive) repo rule; it's fetched
    ...
    *   `archive_type`: A string, the archive type of the downloaded file (Same
        as [`type` on `http_archive`](/versions/9.1.0/rules/lib/repo/http#http_archive-type)).
...
*   If `type` is `local_path`, this module version is backed by a
    [`local_repository`](/versions/9.1.0/rules/lib/repo/local#local_repository) repo rule;
